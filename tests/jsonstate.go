...
func GetTransaction(test map[string]interface{}) (*common.Transaction, error) {
	...
	if m, ok := test["transaction"]; ok {
		if v, ok := m.(map[string]interface{}); ok {
			...
			if tx.Data, err = strToBytes(v,"data"); err != nil { return nil, err }
			if tx.GasLimit, err = strToBigInt(v,"gasLimit"); err != nil { return nil, err }
			if tx.GasPrice, err = strToBigInt(v,"gasPrice"); err != nil { return nil, err }
			if tx.Value, err = strToBigInt(v,"value"); err != nil { return nil, err }
			if tx.Nonce, err = strToUint64(v,"nonce"); err != nil { return nil, err }
			if tx.To, err = strToAddressOpt(v,"to"); err != nil { return nil, err }
			return tx, nil
		}
	}
...
func GetSecretKey(test map[string]interface{}) ([]byte, error){
	...
	if m, ok := test["transaction"]; ok {
		if v, ok := m.(map[string]interface{}); ok {
			if secretKey, err = strToBytes(v,"secretKey"); err != nil { return nil, err }
...
func FillBlockInfo(test map[string]interface{}, blockInfo *common.BlockInfo) error {
	...
	if m, ok := test["env"]; ok {
		if env, ok := m.(map[string]interface{}); ok {
			if blockInfo.Coinbase, err = strToAddress(env, "currentDifficulty"); err != nil {
				return err
			}
